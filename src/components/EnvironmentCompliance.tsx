...
type EnvironmentComplianceProps = {
  /** PDFs to show. Empty or omitted → component renders nothing. */
  documents?: ComplianceDoc[];
  /** Project name, used in the header copy. */
  projectName?: string;
  /** RERA number shown in the viewport footer. */
  reraNumber?: string;
  id?: string;
  showHeader?: boolean;
  showMetrics?: boolean;
...
export default function EnvironmentCompliance({
  ...
  /* ── Hard bail: no documents means the section does not exist ── */
  if (!documents || documents.length === 0) return null;

  /* Clamp in case the list shrank between renders */
  const safeIndex = Math.min(activeDocIndex, documents.length - 1);
  const activeDoc = documents[safeIndex];

  return (
    <section
      id={id}
      className={`relative overflow-hidden text-[#1c2b1e] font-sans selection:bg-[#c9a84c]/20 selection:text-[#1c2b1e] ${
        withShell
          ? "bg-[#f8f5ef] rounded-3xl mx-2 my-2 border border-white/80 shadow-sm py-24 lg:py-32 px-6 lg:px-12"
          : "py-16 px-0"
      } ${className}`}
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {showHeader && (
          ...
          >
            <div>
              ...
              <h2 className="font-playfair text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-[#1c2b1e] leading-[1.05]">
                Environmental <br />
            ...
            <div className="flex flex-col items-start md:items-end justify-end">
              <p className="text-[#1c2b1e]/70 text-sm md:text-base max-w-sm leading-relaxed mb-4 md:text-right">
                We believe legal transparency is the fifth element of luxury. Access
                the verified Ministry of Environment filings and audit reports
                {projectName ? ` for ${projectName}` : ""} below.
              </p>
        ...
        >
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            <div className="flex items-center justify-between px-1">
              ...
              <span className="text-xs font-mono text-[#c9a84c]">
                [{documents.length} FILED REPORT{documents.length > 1 ? "S" : ""}]
              </span>
            </div>
            ...
            {documents.map((doc, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {isSelected && (
                    <motion.div
                      layoutId={`${id}-activeDocIndicator`}
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <span
                        className={`text-[10px] font-mono tracking-wider uppercase px-2.5 py-0.5 rounded-md ${
                          isSelected
                            ? "bg-white/10 text-[#e2c97e]"
                            : "bg-[#1c2b1e]/5 text-[#1c2b1e]/60"
                        }`}
                      >
                        {doc.authority}
                      </span>
                      ...
                      >
                        <span
                          ...
                          style={{
                            backgroundColor: isSelected ? "#10B981" : doc.statusColor,
                          }}
                        />
                        {doc.status}
                      </span>
                    ...
                    <p
                      className={`text-xs font-semibold uppercase tracking-wider mb-4 font-jakarta ${
                  ...
                  <div
                    className={`pt-4 border-t flex items-center justify-between text-xs font-jakarta ${
                      ...
                      <span
                        className={`text-lg transition-transform duration-300 ${
                          isSelected
                            ...
                            : "text-[#1c2b1e]/30 group-hover:translate-x-1"
                        }`}
                      >
                        →
                      </span>
            ...
            <div className="p-5 rounded-2xl bg-white/50 border border-white/60 text-xs text-[#1c2b1e]/70 flex items-center justify-between gap-4">
              <div>
                <p className="font-bold text-[#1c2b1e] mb-0.5">
                  Need hard copies for legal verification?
                </p>
                <p>
                  Official stamped documents are available at our Pune head office
                  upon request.
                </p>
              </div>
              <Link
                href="/#contact"
                className="px-3.5 py-2 rounded-xl bg-[#1c2b1e] text-white font-bold text-[10px] uppercase tracking-widest flex-shrink-0 hover:bg-[#c9a84c] hover:text-[#1c2b1e] transition-colors"
              >
                Inquire
              </Link>
            </div>
          ...
          <div className="lg:col-span-7 rounded-3xl bg-white/80 backdrop-blur-2xl border border-white p-3 sm:p-4 shadow-[0_25px_60px_rgba(28,43,30,0.08)] flex flex-col">
            ...
            <div className="relative w-full h-[550px] sm:h-[650px] bg-gray-900 rounded-b-2xl overflow-hidden shadow-inner flex items-center justify-center">
              ...
              {/* Mobile Fallback Overlay Prompt */}
            ...
            <div className="pt-3 px-3 flex items-center justify-between text-[11px] font-mono text-[#1c2b1e]/50 uppercase tracking-widest">
              <span>{reraNumber ? `RERA REG: ${reraNumber}` : "RERA REGISTERED // PUNE"}</span>
        ...
        {/* =====================================================================
            3. SUSTAINABILITY IMPACT RIBBON
           ===================================================================== */}
        {showMetrics && (
          <div className="rounded-3xl bg-white/70 backdrop-blur-xl border border-white p-8 lg:p-12 shadow-[0_15px_40px_rgba(28,43,30,0.05)] relative overflow-hidden">
            ...
            <div className="mb-8 text-center max-w-xl mx-auto">
              ...
              <h3 className="font-playfair text-2xl sm:text-3xl font-bold text-[#1c2b1e]">
